...
def propose_normalization_spec(
    ...
    *,
    ...
    if not use_ai or not settings.get("openai_api_key"):
...
def _snapshots_for_package(
    ...
    role_map = _merged_file_role_map(settings, category_key, dict(file_config))
    snapshots: list[dict[str, Any]] = []
    for slot_id, filename in sorted(file_config.items()):
        snap = workbook_snapshot(input_dir / filename)
        snap["slot_id"] = slot_id
        snap["source_role"] = role_map.get(slot_id, SourceRole.UNKNOWN.value)
...
def _heuristic_metric_source(
    ...
    for column in columns:
        ...
        if any(_looks_numeric(v) for v in samples):
            metric_mappings[_stat_key(column)] = column
